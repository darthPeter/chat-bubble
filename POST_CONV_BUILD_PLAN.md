...
- **Step 4 — Token Endpoint timer + Twilio filter (together, LAST).** ✅ **LIVE 2026-06-30.** Token Endpoint Create Conversation sets `Timers.Closed=PT10M` (verified: new conv gets `date_closed`, no `date_inactive`). Twilio service filters now `[onMessageAdded, onConversationStateUpdated]` (added, not replaced). Verified: real closed conv → dispatcher envelope correct; real message round-trip still replies. **Pipeline fully live.** Consumer contract: `HANDOVER-postpro-brain-2026-06-30.md`.
