...
- [ ] **Localize `Prepare Safe Reply` (guardrail-blocked path)** — Message Handler's `Prepare Safe Reply` node returns an English-only line (`"I'm here to help you with your questions. Could you please rephrase your message?"`) to every client. The `Prepare Reply` failure fallback was made per-client on 2026-06-16 (EN: digishares/atlaschat; CZ: alkoholcz/pompo/ladenta) but the guardrail path was deliberately left out of that fix's scope. Mirror the same per-client `FALLBACK` map there for consistency so Czech clients (Alkohol, Pompo, LaDenta/Iva) get a Czech rephrase prompt when a message is blocked. Small, low-risk (~15 min). Flagged by reviewer 2026-06-16.
- [ ] **Page URL metadata in welcome message** — widget reads `window.location.href` of the host page, appends to welcome trigger as `[system] generate welcome message | page_url=<url>`. Brains opt in by parsing; brains that ignore it behave as today (backwards-compatible by design). Example use case: Atlas Copco multi-locale site — brain parses `/fr-fr/itba/products` → responds in French already in the welcome message. **V1 locked (Petr 2026-06-10):** (a) **static** — capture URL at welcome-message time only; SPA-navigation awareness is V2. (b) **inline** transport — embed in welcome trigger string, NOT Twilio Conversation `attributes`. Attributes path noted as future V2 (architecturally cleaner, brain reads via Twilio API). **Still open for implementation time:** (1) **privacy** — strip query strings by default to avoid UTM/PII leaks; per-client opt-in to keep query if needed; (2) **per-client opt-in** — `data-send-page-url` attribute on widget script, default on (cost is just a longer trigger string for brains that ignore). Effort: small, ~1-2h widget-side. Risk: low. Brain-side prompt work scales per client that wants the feature. **Cross-link to live-agent (Petr 2026-06-10):** for the live-agent handoff flow, knowing the user's current page is MORE important than the brain-locale use case (agent needs current-page context in the support dashboard). When live-agent reaches step 3+, doing the Twilio `attributes` path THEN — with `page_url` tracked live (SPA-aware) on the conversation — unlocks both features off the same plumbing. The inline-welcome V1 here becomes a quick stepping stone rather than long-term truth.
